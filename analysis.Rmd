...
We filter the data to remove unused timeperiods. Phi is the probablity that, given a initial TP and Purpose you return in another purpose or TP.
...
To ease the comparison we are goingo to add to the matrices the typical 3 car user classes EB, HBW, HBO using the purspose <-> UC correspondance file in the data folder.
...
HBmat %>%
  ...
  ggplot(aes(x= Bound, y= trips, label = trips,fill = as.factor(UC))) +
