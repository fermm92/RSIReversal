...
factor2 <- phi %>% group_by(HO,DO) %>% summarise(f2 = 1/sum(PHI1)) # to make the sum of the return 1
...
HBmat$Bound <- factor(HBmat$Bound, levels = c("Out","In"))
...
HBmat2$Bound <- factor(HBmat2$Bound, levels = c("Out","In"))
...
for (tp in (1:3)){
  for (uc in (1:3)){
    fwrite(test[test$TP == tp & test$UC == uc,], 
           file = paste0("./output/full_matrices/RSI_Car_UC",uc,"_TS",tp,".dat"))
  }
}
```
